...
const App: React.FC = () => {
    ...
    const [allSections, setAllSections] = useState<SectionData[]>([]); // Store full data for client-side search
    ...
    useEffect(() => {
        const fetchJobs = async () => {
            ...
            setAllSections(data); // Save master copy
            setSections(data);    // Set visible data
            setIsLoading(false);
        };
        fetchJobs();
    }, []);

    // Handle Search (Client-Side Synchronous)
    const handleSearch = (query: string) => {
        ...
        if (!query.trim()) {
            setSections(allSections); // Reset to full list
            return;
        }

        const lowerQuery = query.toLowerCase();

        // Filter synchronously from master data
